...
export default function Home({ language, setLanguage }) {
  ...
  const localTodos = window.localStorage.getItem('todos') ? JSON.parse(window.localStorage.getItem('todos')) : [];

  const localTrash = window.localStorage.getItem('trash') ? JSON.parse(window.localStorage.getItem('trash')) : [];
  ...
  useEffect(() => {
    ...
    const countValue = Math.max(...allInOneID) === -Infinity ? -1 : Math.max(...allInOneID);
